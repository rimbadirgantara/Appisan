      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            <td>{{$du['username']}}<br>
              <span
                class="badge rounded-pill badge-soft-{{($du['level'] == 'admin' ? 'success' : 'warning')}}">{{$du['level']}}</span>
            </td>
            <td>Kelas {{$du['kelas']}}</td>
            <td>{{$du['nama_sekolah']}}</td>
            <td class="text-end">
              <div>
                <a href="/admin/siswa/{{$du['id_user']}}/edit" class="btn btn-sm btn-warning" type="button" title="Edit" data-bs-placement="top"
                  data-bs-toggle="tooltip" data-bs-target="#edit-user"><span class="fas fa-edit"></span></a>

                <a href="/admin/siswa/{{$du['id_user']}}/hapus" class="btn btn-sm btn-danger" type="button"
    ...
    <div class="modal-content position-relative">
      <div class="modal-body p-0">
        <div class="rounded-top-lg py-3 ps-4 pe-6 bg-light">
          <h4 class="mb-1" id="modalExampleDemoLabel">Tambah User</h4>
        </div>
        <div class="p-4 pb-0">
